...
import { isValidEmail, validatePasswordStrength } from 'src/utils/index'
...
const syncFromQuery = () => {
  formData.token = typeof route.query.token === 'string' ? route.query.token : ''
  formData.email = typeof route.query.email === 'string' ? route.query.email : ''
}

watch(() => [route.query.token, route.query.email], syncFromQuery, { immediate: true })
...
const handleSubmit = async () => {
  ...
  if (!formData.token) {
    errors.value = { general: 'Reset token is missing from the link. Please request a new reset email.' }
    return
  }
    ...
    <div class="auth-shell">
      <q-card class="auth-card">
        ...
        <q-card-section v-if="message" class="q-pb-none">
          <p class="text-positive q-mb-none">{{ message }} Redirecting to login...</p>
        </q-card-section>
